...
async def test_upsert_today_updates_fields(create_user_func) -> None:
    ...
    await service.upsert_today(DailyCheckinUpdate(mood=3, cravingScore=5), user)
    response = await service.get_today(user)

    assert response.mood == 3
    assert response.cravingScore == 5
...
async def test_sober_streak_counts_consecutive_safe_days(create_user_func) -> None:
    ...
    await service.upsert_today(DailyCheckinUpdate(date=two_days_ago, substanceUse={"status": "used"}), user)
    ...
    assert shaped.soberStreakDays == 2  # today + yesterday; used day ends the streak
